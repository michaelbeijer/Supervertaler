...
class Supervertaler:
    ...
    def __init__(self, root):
        ...
        self.root = root
        self.root.title("Supervertaler v3.7.7 - AI-Powered CAT Tool")
        ...
        # Status
        self.log("Supervertaler v3.7.7 ready. Import a DOCX file to begin.")
    ...
    def setup_ui(self):
        ...
        tools_menu.add_command(label="TMX Editor", command=self.open_tmx_editor_window)
        tools_menu.add_separator()
    ...
    def create_start_screen(self):
        ...
        tk.Label(center_frame, text="v3.7.7", 
    ...
    def create_tabbed_assistance(self):
        ...
        if self.assist_visible_panels.get('tmx_editor', True):
            self.assist_tabs.append({
                'key': 'tmx_editor',
                'name': '📝 TMX Editor',
                'short': 'TMX Ed.',
...
✓ Quality assurance checks"""
    ...
    def create_tmx_editor_tab(self, parent):
        """Create TMX Editor tab content - embedded TMX editor"""
        from modules.tmx_editor import TmxEditorUI
        
        # Info header
        info_frame = tk.Frame(parent, bg='#e8f4f8', relief='solid', borderwidth=1)
        info_frame.pack(fill='x', padx=5, pady=5)
        
        tk.Label(info_frame, text="📝 TMX Editor", font=('Segoe UI', 10, 'bold'),
                bg='#e8f4f8').pack(anchor='w', padx=10, pady=5)
        tk.Label(info_frame, text="Edit translation memory files directly - inspired by Heartsome TMX Editor",
                font=('Segoe UI', 9), bg='#e8f4f8', fg='#666').pack(anchor='w', padx=10, pady=(0, 5))
        
        # Quick actions toolbar
        quick_toolbar = tk.Frame(parent, bg='#fff3e0', relief='solid', borderwidth=1)
        quick_toolbar.pack(fill='x', padx=5, pady=(0, 5))
        
        tk.Label(quick_toolbar, text="Quick Actions:", bg='#fff3e0',
                font=('Segoe UI', 9, 'bold')).pack(side='left', padx=10, pady=5)
        
        tk.Button(quick_toolbar, text="📂 Open TMX",
                 command=lambda: self.tmx_editor_embedded.open_tmx() if hasattr(self, 'tmx_editor_embedded') else None,
                 bg='#4CAF50', fg='white', font=('Segoe UI', 8)).pack(side='left', padx=2)
        
        tk.Button(quick_toolbar, text="🪟 Open in Separate Window",
                 command=self.open_tmx_editor_window,
                 bg='#2196F3', fg='white', font=('Segoe UI', 8)).pack(side='left', padx=2)
        
        tk.Button(quick_toolbar, text="💾 Save",
                 command=lambda: self.tmx_editor_embedded.save_tmx() if hasattr(self, 'tmx_editor_embedded') else None,
                 bg='#FF9800', fg='white', font=('Segoe UI', 8)).pack(side='left', padx=2)
        
        # Embed TMX Editor
        try:
            self.tmx_editor_embedded = TmxEditorUI(parent=parent, standalone=False)
            self.log("✓ TMX Editor embedded in assistant panel")
        except Exception as e:
            tk.Label(parent, text=f"⚠️ Failed to load TMX Editor: {str(e)}",
...
"""
    ...
    def open_tmx_editor_window(self):
        """Open TMX Editor in a separate window"""
        from modules.tmx_editor import TmxEditorUI
        
        try:
            # Create standalone TMX Editor window
            tmx_window = tk.Toplevel(self.root)
            tmx_window.title("TMX Editor - Supervertaler")
            tmx_window.geometry("1200x700")
            tmx_window.transient(self.root)
            
            # Create TMX Editor (embedded mode in new window)
            tmx_editor = TmxEditorUI(parent=tmx_window, standalone=False)
            
            # Handle window close
            def on_close():
                if tmx_editor.tmx_file and tmx_editor.tmx_file.is_modified:
            ...
            tmx_window.protocol("WM_DELETE_WINDOW", on_close)
            
            self.log("✓ TMX Editor opened in separate window")
            
        except Exception as e:
            messagebox.showerror("Error", f"Failed to open TMX Editor:\n{str(e)}")
            self.log(f"❌ TMX Editor error: {str(e)}")
