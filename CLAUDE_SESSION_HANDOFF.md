...
TOTAL RECALL (Archive) = Giant SQLite DB (millions of segments)
...
PROJECT TM (Working Set) = In-memory SQLite or Python dict
Contains only segments that match project source text:
- Exact matches (100%)
- Fuzzy matches (>=75%)
...
3. Reduce debounce delays - 150ms to 50ms, 300ms to 100ms
...
Option A: Update PowerShell Script to just use single spec
Option B: Create Python build_release.py that works cross-platform
...
## Session Continuation - January 31, 2026

### Progress Made

1. **ProjectTM class implemented** - [modules/project_tm.py](modules/project_tm.py)
   - In-memory SQLite database for instant lookups
   - FTS5 index for fuzzy matching
   - Deduplication to avoid repeated TM entries

2. **ExtractTM class implemented** - [modules/extract_tm.py](modules/extract_tm.py)
   - Persistent .svtm files saved next to project
   - Exports to TMX
   - Reloads on subsequent project opens

3. **Thread-safety issues FIXED**:
   - `check_same_thread=False` on SQLite connections
   - Created local cursors in `_search_single_tm_fuzzy()` to avoid "Recursive use of cursors" error
   - Thread-local DatabaseManager for background extraction worker

4. **Background extraction worker** - [Supervertaler.py:~7577](Supervertaler.py#L7577)
   - Creates thread-local DatabaseManager with `connect()` call
   - Passes `log_callback` for visibility in log window

### Current Issue: TM Search Returns 0 Matches

**Symptoms:**
- ProjectTM extraction completes but finds 0 segments
- TMs exist with data (patents has 660,937 entries)
- FTS5 index has 865,803 entries
- Language pairs in DB: `('Dutch', 'English'), ('en', 'nl'), etc.`
- Search direction: `nl -> en`

**Diagnostic logging added to database_manager.py:**
```python
# Line ~955: Logs FTS test query without lang filter
# Line ~1063: Logs exact params being used for each TM search
# Line ~1071: Logs how many FTS candidates returned per TM
```

**Suspected causes (in order of likelihood):**
1. Language filtering mismatch - `get_lang_match_variants('nl')` returns `['nl', 'Dutch']` but might not match actual DB values
2. FTS5 query syntax issue - Using `"word1" OR "word2"` format
3. FTS5 index out of sync with main table (unlikely - triggers exist)

**Next steps to debug:**
1. Run application and check new diagnostic logs
2. Verify FTS test query (without lang filter) returns results
3. Check exact SQL params being passed vs DB content
4. Try removing language filter temporarily to isolate issue

### Key Files Modified

| File | Changes |
|------|---------|
| `modules/project_tm.py` | New file - in-memory ProjectTM |
| `modules/extract_tm.py` | New file - persistent ExtractTM |
| `modules/database_manager.py` | Thread-safe cursors, diagnostic logging |
| `Supervertaler.py` | extraction_worker with thread-local DB |
...
# Look for log lines starting with "[TM Search]" and "[ProjectTM]"
# Key things to check:
# - "FTS test query 'xyz' returned N matches (no lang filter)"
# - "TM ['patents']: FTS returned N candidates"
# - "Lang filter params: [...]"
```
