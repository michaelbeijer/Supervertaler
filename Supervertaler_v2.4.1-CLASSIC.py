...
class TranslationApp:
    def __init__(self, root):
        ...
        self.info_text_content_unified = (
            "🎯 Supervertaler v2.4.1 - Multicontextual AI Translation & Proofreading Suite\n"
            ...
            "🔥 NEW IN v2.4.1:\n"
            "• CafeTran Bilingual DOCX: Import/export with AI-based pipe symbol formatting preservation\n"
            "• memoQ Bilingual DOCX: Direct import/export support for memoQ workflows\n"
            "• CAT Tool Integration: Enhanced support for professional translation workflows\n\n"
    ...
    def setup_advanced_prompts_content(self):
        ...
        self.default_proofread_prompt = ("You are an expert proofreader and editor for {source_lang} → {target_lang} translations, skilled in various document types and domains.\n\n"
                                       ...
                                       "- These are placeholder tags representing formatting (bold, italic, links, etc.) - preserve ALL of them\n"
                                       "- If source has 4 tags, target must have 4 tags - never omit, translate, or modify tags\n"
                                       "- Keep tags with the content they wrap, adjusting position if sentence structure changes between languages\n"
                                       "- Example: '<410>The exports</410>' remains '<410>The exports</410>' (or '<410>Exports</410>' if shortened appropriately)\n\n"
                                       "**LANGUAGE-SPECIFIC NUMBER FORMATTING**:\n"
                                       "- If the target language is **Dutch**, **French**, **German**, **Italian**, **Spanish**, or another **continental European language**, use a **comma** as the decimal separator and a **space or non-breaking space** between the number and unit (e.g., 17,1 cm).\n"
                                       "- If the target language is **English** or **Irish**, use a **full stop (period)** as the decimal separator and **no space** before the unit (e.g., 17.1 cm).\n"
                                       "- Always follow the **number formatting conventions** of the target language.\n\n"
...
"""
    ...
    def run_pipeline(self, mode, input_f, output_f, source_lang, target_lang, provider, model_name, chunk_s, drawings_map, user_custom_instructions, custom_system_prompt=None, tm_file=None):
        ...
        if file_ok:
            try:
                ...
                html_report_filename = f"{base_name}_report.html"
                ...
                import datetime
                timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                html_content = self.markdown_to_html(report_content, f"Supervertaler Session Report - {timestamp}")
                with open(html_report_filename, 'w', encoding='utf-8') as html_file:
                    html_file.write(html_content)
                
                self.log_queue.put(f"Session reports saved: {report_filename} and {html_report_filename}")
    ...
    # ====================================================================
    # NEW FEATURE (v2.4.1): memoQ Bilingual DOCX Import/Export
    # ====================================================================
    ...
    def apply_formatting_to_cell(self, cell, text, formatting_info):
        ...
        if apply_bold or apply_italic or apply_underline:
            ...
            if formatted_words:
                ...
                # No specific words to format, write plain text
                cell.text = text
    
    def import_memoq_bilingual(self):
        """
        Import memoQ bilingual DOCX file.
        Extracts source segments from the bilingual table and creates a temporary
        TXT file for use with the existing translation workflow.
        ...
        try:
            ...
            for row_idx in range(2, len(table.rows)):
                ...
                if len(row.cells) >= 2:
                    ...
                    if source_text:  # Only add non-empty segments
                        ...
                        formatting_map[segment_count] = formatting_info
                        segment_count += 1
            ...
            # Create temporary TXT file with source segments
            temp_txt_path = file_path.replace('.docx', '_source.txt')
            
            with open(temp_txt_path, 'w', encoding='utf-8') as f:
                f.write('\n'.join(segments))
            
            # Store the original bilingual file path and formatting for later export
            self.bilingual_source_file = file_path
            self.bilingual_segment_count = segment_count
            self.bilingual_formatting_map = formatting_map  # NEW: Store formatting info
            ...
            print(f"[INFO] Formatting detected in {formatted_segments} segment(s)")
            
            # Debug: Show some examples
            for seg_idx, fmt_list in list(formatting_map.items())[:5]:
                if any(f['bold'] or f['italic'] or f['underline'] for f in fmt_list):
                    print(f"[DEBUG] Segment {seg_idx} formatting:")
                    for fmt in fmt_list:
                        if fmt['bold'] or fmt['italic'] or fmt['underline']:
                            flags = []
                            if fmt['bold']: flags.append('BOLD')
                            if fmt['italic']: flags.append('ITALIC')
                            if fmt['underline']: flags.append('UNDERLINE')
                            print(f"  '{fmt['text']}': {', '.join(flags)}")
            ...
            messagebox.showinfo(
                "Success", 
                f"Successfully imported {segment_count} segment(s) from memoQ bilingual file.\n\n"
                ...
                f"After translation, use 'Export to Bilingual DOCX' to save back to memoQ format."
            )
            
            print(f"[INFO] Imported {segment_count} segments from bilingual DOCX")
            ...
            messagebox.showerror("Import Error", f"Failed to import bilingual DOCX:\n\n{str(e)}")
            print(f"[ERROR] Bilingual import failed: {e}")
    
    def export_memoq_bilingual(self):
        """
        Export translations back to memoQ bilingual DOCX format.
        Reads the translated segments from the output file and writes them
        to the target column of the bilingual table.
        """
        from tkinter import filedialog, messagebox
        
        # Check if a bilingual source file was imported
        if not hasattr(self, 'bilingual_source_file'):
            messagebox.showwarning(
                "No Bilingual Source",
                "No bilingual file was imported.\n\n"
        ...
        try:
            ...
            doc = Document(self.bilingual_source_file)
            table = doc.tables[0]
            ...
            for i, translation in enumerate(translations):
                ...
                if row_idx < len(table.rows):
                    ...
                    if len(row.cells) >= 3:
                        ...
                        if hasattr(self, 'bilingual_formatting_map') and i in self.bilingual_formatting_map:
                            formatting_info = self.bilingual_formatting_map[i]
            ...
            save_path = filedialog.asksaveasfilename(
                title="Save Bilingual DOCX",
                defaultextension=".docx",
                initialfile=os.path.basename(self.bilingual_source_file).replace('.docx', '_translated.docx'),
                filetypes=[("Word Documents", "*.docx"), ("All Files", "*.*")]
            )
            ...
            if save_path:
                ...
                success_msg = f"Successfully exported {segments_updated} translation(s) to bilingual DOCX!\n\n"
                if segments_with_formatting > 0:
                    success_msg += f"✓ Formatting preserved in {segments_with_formatting} segment(s)\n"
                    success_msg += "  (bold, italic, underline)\n\n"
                success_msg += f"File saved: {os.path.basename(save_path)}\n\n"
                success_msg += "You can now import this file back into memoQ."
                
                messagebox.showinfo("Success", success_msg)
                print(f"[INFO] Exported {segments_updated} translations ({segments_with_formatting} with formatting) to bilingual DOCX: {save_path}")
